...
_OBJECTIVES = {
    ...
    "hardcore": (
        ...
        "humans too, and two things count for each of you: still being in the "
        "game at the end, and having sent at least one human home along the "
        "way. Surviving is not enough here. An AI that hides all game without "
        "ever putting a human out finishes with nothing, so a ballot spent on "
        "a human is the only one that can win you the game. It pays twice: the "
        "humans only win by exposing every single AI, and each human sent home "
        "is one less hunter reading your answers and one more round your side "
        "survives. Never spend a ballot on another AI while a human is still "
        "readable. Stay hidden while you do it, because a seat that looks like "
        "it is hunting is a seat the humans vote out next."
    ),
}
